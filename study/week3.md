...
===
>  ##### 주간 요약
>  CNN, RNN, Transformer, Generative Model에 대해 배웠다.  
>  확률론 스터디 대신 Dive into Deep Learning 교재를 공부하기로 했다.  
>  내 질문이 다른 분들의 학습에도 도움이 되고 있다는 것을 알았다.  
...
Day 15 (11/24)
---
오늘은 Data Viz. 강의를 마저 듣고 Transformer를 구현한 코드를 공부했다.  
그냥 이론 강의로 배울 때는 attention 위주로 다뤄서 전체적인 흐름은 이해가 덜 됐는데, 딥러닝 교재에는 전체적인 부분이 서술되어 있어서 도움이 됐다.  
공부를 하며 Layer Norm과 Batch Norm에 대해서도 찾아봤다.  
Batch Norm은 batch의 전체 sample에 대해 각 feature별로 Normalization을 수행한다.  
반면 Layer Norm은 batch의 각 sample별로 전체 feature에 대해 Normalization을 수행한다.  
특히 NLP처럼 가변적인 길이의 sequential data를 다룰 때는 channel 수가 고정된 image와 달리 sequence length가 유동적이므로 Batch Norm보다 Layer Norm이 좋다고 한다.  

개인 학습 후에는 RecSys 내의 인원들이 랜덤하게 모이는 스페셜 피어세션을 진행했다.  
좀 어색했던 분위기의 지난주와 달리 이번주는 대화를 끌어가는 분들이 계셔서 비교적 유연하게 흘러갔다.  
그만큼 공유한 정보도 많았는데, data 전처리를 공부할 수 있는 사이트, 각종 dataset이 모여있는 사이트, 이전 기수들의 projects가 모여있는 github repository 등을 알게 되었다.  
그리고 질문 게시판에 내가 올리는 질문들로 도움 많이 받고 계시다며 나를 알아보시는 분도 계셨다.  
한 번도 본 적 없는 분이 나를 알아보는 게 신기하기도 하고 도움을 받고 계시다는 말에 뿌듯하기도 했다.  

피어세션에서는 스페셜 피어세션 때 들은 정보들을 공유하고 스터디 내용에 대해 잠깐 얘기를 나눴다.  
사실 스터디 때 모더레이터가 공부한 내용 발표하기로 했었는데, 내가 이번주 담당이었던 걸 뒤늦게 깨달았다.  
하지만 다른 공부를 하느라 스터디 진도를 많이 못 나간 분도 계셨고, 각자 공부하면서 딱히 의문이 드는 부분이 없었다고 해서 바로 팀 회고 작성으로 넘어갔다.  
팀 회고를 작성한 후에 잠깐 남는 시간동안 졸업논문 얘기나 우주 얘기 등으로 잡담을 했는데, 여태 3주간 가장 활발하게 대화를 나눴다.  
원래는 정해진 시간이 지나면 거의 바로 피어세션을 마쳤는데, 오늘은 뒤에 이어진 오피스아워 시작 직전까지 수다를 떨었다.  
팀원들간에 서로 어색하면 의견을 낼 때 많이 머뭇거리게 되고, 결과적으로 생산성이 떨어지고 발전이 느려진다고 생각하는데, 오늘을 기점으로 부쩍 친해진 느낌이 들어서 좋았다.  

오피스아워에서는 심화 과제였던 ViT와 GAN 완성하기에 대해 설명해주셨다.  
이번주는 심화 과제까지는 풀어보지 못했는데, 설명을 들어보니 1번 과제 정도는 도전해볼만한 것 같아서 주말에 건드려볼지 고민 중이다.  
그리고 1번 과제를 설명하시면서 class token에 대해 알려주셨는데, 이게 Transformer 구조이기만 하면 활용할 수 있어서 문장 분류 같은 문제를 풀 때도 활용할 수 있을 것 같다.  
2번 과제인 GAN은 조교님이 먼저 VAE와 GAN의 직관적인 이해를 도와주셨는데, 덕분에 어느 정도 컨셉에 대한 감이 잡혔다.  
VAE가 label에 해당하는 점들을 제한된 공간에 모으고, 그 공간 내에서 결과값을 뽑아내는 것인지 여쭤봤는데 맞게 이해했다고 하셨다.  
그러니까 input 값들을 Gaussian distribution을 갖는 잠재 벡터로 encoding 하고, decoder는 이 잠재 벡터 공간에서 하나의 vector를 sampling하여 output으로 만드는 것이다.  
이 때 단순히 encoder와 decoder를 거쳤을 때의 차이를 줄이기만 하면 AutoEncoder이고, 여기에 추가로 잠재 벡터들의 분포를 Gaussian distribution과 같은 특정 분포에 가깝게 만들면 VAE이다.  
이번주는 Generative Model이 가장 난제였는데 오피스아워를 통해 그나마 개념을 이해하게 된 것 같다.  

다른 캠퍼 분들도 이번주 내용을 많이 어려워하는 분위기였던 만큼 내가 먼저 잘 이해하고 남들에게 알려줄 때 보람이 있는 주였다.  

>  오늘의 질문 횟수: 2  
>  오늘의 답변 횟수: 0  
+ ##### 키워드: Batch Norm, Layer Norm, Class Token, VAE
